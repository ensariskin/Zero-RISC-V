...
class StatisticsPanel:
    ...
    def __init__(self, parent):
        self.parent = parent
        self.frame = ttk.LabelFrame(parent, text="📊 Comparison Statistics", padding="10")
        self.setup_widgets()
    ...
    def setup_widgets(self):
        ...
        ttk.Label(left_frame, textvariable=self.core_entries_var).pack(anchor=tk.W, pady=2)
        ttk.Label(left_frame, textvariable=self.spike_entries_var).pack(anchor=tk.W, pady=2)
        ttk.Label(left_frame, textvariable=self.loops_removed_var).pack(anchor=tk.W, pady=2)
        ttk.Label(left_frame, textvariable=self.lcs_length_var).pack(anchor=tk.W, pady=2)
        ...
        ttk.Label(right_frame, textvariable=self.matches_var).pack(anchor=tk.W, pady=2)
        ttk.Label(right_frame, textvariable=self.deletions_var).pack(anchor=tk.W, pady=2)
        ttk.Label(right_frame, textvariable=self.insertions_var).pack(anchor=tk.W, pady=2)
        ttk.Label(right_frame, textvariable=self.match_pct_var).pack(anchor=tk.W, pady=2)
...
class DiffViewer:
    ...
    def __init__(self, parent):
        self.parent = parent
        self.frame = ttk.LabelFrame(parent, text="🔍 Side-by-Side Difference Viewer", padding="5")
    ...
    def setup_widgets(self):
        """Setup side-by-side diff viewer widgets"""
        # Create toolbar
        ...
        ttk.Button(search_frame, text="Find", command=self.search_text).pack(side=tk.LEFT, padx=(0, 10))
        
        # Font size controls
        font_frame = ttk.Frame(toolbar)
        font_frame.pack(side=tk.LEFT, padx=(20, 0))
        
        ttk.Label(font_frame, text="Font Size:").pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Button(font_frame, text="🔍+", command=self.increase_font).pack(side=tk.LEFT, padx=(5, 2))
        ttk.Button(font_frame, text="🔍-", command=self.decrease_font).pack(side=tk.LEFT, padx=(2, 0))
        
        # Navigation buttons
        nav_frame = ttk.Frame(toolbar)
        nav_frame.pack(side=tk.LEFT, padx=(20, 0))
        
        ttk.Button(nav_frame, text="⬆️ Prev Diff", command=self.prev_diff).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(nav_frame, text="⬇️ Next Diff", command=self.next_diff).pack(side=tk.LEFT, padx=(0, 5))
        ...
        # Left panel - Core Log
        left_panel = ttk.LabelFrame(comparison_frame, text="🔧 Core Log", padding="5")
        left_panel.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))
        
        # Right panel - Spike Log  
        right_panel = ttk.LabelFrame(comparison_frame, text="⚡ Spike Log", padding="5")
...
class RISCVLogComparatorGUI:
    ...
    def setup_widgets(self):
        ...
        ttk.Label(title_frame, text="🚀 Professional RISC-V Log Comparator", 
                 style='Title.TLabel').pack(side=tk.LEFT)
        
        ttk.Label(title_frame, text="v3.0 - Classical LCS Algorithm", 
        ...
        # Core file
        core_frame = ttk.Frame(file_frame)
        core_frame.pack(fill=tk.X, pady=(0, 5))
        
        ttk.Label(core_frame, text="Core Log:", width=12).pack(side=tk.LEFT)
        self.core_file_var = tk.StringVar(value="No file selected")
        self.core_label = ttk.Label(core_frame, textvariable=self.core_file_var, 
                                   background=self.theme['entry_bg'], 
                                   foreground=self.theme['entry_fg'], 
                                   relief='sunken', padding="5")
        self.core_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 5))
        
        ttk.Button(core_frame, text="Browse", 
                  command=self.select_core_file).pack(side=tk.RIGHT)
        
        # Spike file
        spike_frame = ttk.Frame(file_frame)
        spike_frame.pack(fill=tk.X, pady=(5, 0))
        
        ttk.Label(spike_frame, text="Spike Log:", width=12).pack(side=tk.LEFT)
        self.spike_file_var = tk.StringVar(value="No file selected")
        self.spike_label = ttk.Label(spike_frame, textvariable=self.spike_file_var, 
                                    background=self.theme['entry_bg'], 
                                    foreground=self.theme['entry_fg'], 
                                    relief='sunken', padding="5")
        self.spike_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 5))
        
        ttk.Button(spike_frame, text="Browse", 
                  command=self.select_spike_file).pack(side=tk.RIGHT)
        
        # Action buttons
        action_frame = ttk.Frame(main_frame)
        action_frame.pack(fill=tk.X, pady=(0, 10))
        
        self.compare_btn = ttk.Button(action_frame, text="🔍 Compare Logs", 
                                     command=self.compare_logs, 
                                     style='Accent.TButton',
                                     state='disabled')
        self.compare_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        ttk.Button(action_frame, text="🌙 Toggle Theme", 
        ...
        # Status bar
        self.status_var = tk.StringVar(value="Ready - Select log files to compare")
    ...
    def setup_console(self, parent):
        ...
        # Add initial message
        self.log_to_console("🚀 Professional RISC-V Log Comparator v3.0 Initialized")
        self.log_to_console("📚 Using Classical LCS Dynamic Programming Algorithm")
        self.log_to_console("👨‍💻 Created by: Şükrü (Advanced AI Assistant)")
        self.log_to_console("📁 Select your log files to begin comparison...")
    ...
    def select_core_file(self):
        ...
        if filename:
            ...
            self.core_file_var.set(os.path.basename(filename))
            self.log_to_console(f"📂 Core log selected: {os.path.basename(filename)}")
    ...
    def select_spike_file(self):
        ...
        if filename:
            ...
            self.spike_file_var.set(os.path.basename(filename))
            self.log_to_console(f"📂 Spike log selected: {os.path.basename(filename)}")
            self.update_compare_button()
    ...
    def update_compare_button(self):
        ...
        if self.core_file and self.spike_file:
            ...
            self.status_var.set("Ready to compare - Click 'Compare Logs' button")
        else:
    ...
    def toggle_theme(self):
        ...
        accent_active = '#16c79a' if self.current_theme == 'dark' else '#1565c0'
        style.configure('Accent.TButton', 
                       background=self.theme['accent'], 
                       foreground='white',
                       font=('Segoe UI', 10, 'bold'),
        ...
        # Update diff viewer colors properly for both text widgets
        if hasattr(self, 'diff_viewer'):
            ...
            # Update line number widgets
            line_bg = '#404040' if self.current_theme == 'dark' else '#f0f0f0'
            line_fg = '#888888' if self.current_theme == 'dark' else '#666666'
            
            self.diff_viewer.core_lines.configure(bg=line_bg, fg=line_fg)
            self.diff_viewer.spike_lines.configure(bg=line_bg, fg=line_fg)
