%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 3.4 VERİ KONTROL VE YAYINLAMA AŞAMASI (YENİDEN YAPILANDIRILMIŞ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Veri Kontrol ve Yayınlama Aşaması}\label{sec:data_control}

Veri kontrol ve yayınlama aşaması, Tomasulo algoritmasının operand çözümleme ve komut gönderme mekanizmalarını içermektedir \cite{tomasulo}. Bu aşama, kod çözme ve yeniden adlandırma aşamasından gelen komutların operand değerlerini okumakta ve operandlar hazır değilse etiket tabanlı bekleme yapmaktadır. Tüm operandlar hazır olduğunda komutlar işlevsel birimlere gönderilmekte ve ortak veri yolu üzerinden sonuçlar yayınlanarak yeniden sıralama arabelleği güncellenmektedir.
...
Tasarlanan sistemde bu aşama üç ana bileşenden oluşmaktadır: yeniden sıralama arabelleği, rezervasyon istasyonları ve yazmaç dosyası.

\begin{figure}[htbp]
    \centering
    \fbox{\textbf{[GÖRSEL: Veri Kontrol Aşaması Genel Yapısı - RS, ROB, PRF ve CDB Bağlantıları]}}
    \caption{Veri kontrol ve yayınlama aşaması mekanizmaları}
    \label{fig:data_control_overview}
\end{figure}

% Fiziksel Yazmaç Alanı - Ana başlık altında anlatılıyor
\label{subsec:phys_reg_space}
...
Tasarlanan sistemde üç ayrı rezervasyon istasyonu bulunmaktadır ve her biri tek bir yürütme birimine bağlıdır. Her rezervasyon istasyonu, işlevsel birimlerin ihtiyaç duyduğu veri ve kontrol bilgilerini saklamaktadır.
...
Yükleme saklama kuyruğu için özel bir eşleştirme gerekmektedir. ALU sonuçları tek bir ortak veri yolu kanalından gelmektedir ve her birim için bir kanal bulunmaktadır. Ancak yükleme saklama kuyruğundan üç ayrı sonuç gelebilmektedir. Etiket değeri üç yalnızca yükleme saklama kuyruğundan beklediğini belirtmekte, hangi girişten geleceğini belirtmemektedir. Çünkü yükleme saklama kuyruğu hangi komutun hangi kanaldan iletileceğine dinamik olarak karar vermektedir. Bu nedenle yükleme saklama kuyruğu için ek olarak yeniden sıralama arabelleği kimliği eşleştirmesi yapılmaktadır.
...
Tasarlanan sistemde altı ortak veri yolu kanalı bulunmaktadır. İlk üç kanal ALU'lardan gelmektedir: sıfırıncı kanal sıfırıncı ALU sonucunu, birinci kanal birinci ALU sonucunu, ikinci kanal ikinci ALU sonucunu taşımaktadır.

Sonraki üç kanal yükleme saklama kuyruğundan gelmektedir. Bu kanallar, her çevrimde en fazla üç yükleme sonucunun veya saklama tamamlanma bildiriminin yayınlanmasına olanak tanımaktadır.
...
Yazmaç dosyası, otuz iki adet otuz iki bitlik yazmaç içeren çok portlu bir yapıdır. Bu yapı, kesinleştirilmiş değerleri tutmaktadır.

Yazmaç dosyası, altı okuma portu ve üç yazma portuna sahiptir. Altı okuma portu, her komut için iki kaynak operand okumak üzere kullanılmaktadır ve üç komut için toplam altı okuma gerekmektedir. Üç yazma portu, kesinleştirme sırasında eş zamanlı yazma için kullanılmaktadır.
...
RISC-V mimarisinde sıfırıncı yazmaç her zaman sıfır değerini taşımaktadır. Yazmaç dosyasında sıfırıncı fiziksel yazmaç, her saat çevriminde sıfıra zorlanmaktadır. Bu sayede herhangi bir komut sıfırıncı yazmaca yazma yapmaya çalışsa bile değer değişmemektedir.
