...
\subsection{IPC Başarımı ve Karşılaştırmalı Analiz}

Tasarlanan üç yollu süperölçekli işlemcinin sağladığı başarım artışını nicelleştirmek adına, aynı mimarinin tek yollu (scalar) versiyonu ile karşılaştırmalı testler yapılmıştır. Simülasyonlar, doğrulama ortamında 10 ns (100 MHz) saat periyodu kullanılarak gerçekleştirilmiştir. Tablo \ref{tab:ipc_results}, Bölüm \ref{subsec:algoritma_testleri}'de tanımlanan deterministik algoritma seti üzerinde elde edilen ortalama performans metriklerini sunmaktadır.

\begin{table}[htbp]
\centering
\caption{1-Yollu ve 3-Yollu Konfigürasyonların Performans Karşılaştırması}
