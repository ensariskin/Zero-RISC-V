...
Tasarlanan üç yollu süperölçekli işlemcinin sağladığı başarım artışını nicelleştirmek adına, aynı mimarinin tek yollu (scalar) versiyonu ile karşılaştırmalı testler yapılmıştır. Tablo \ref{tab:ipc_results}, farklı kategorideki test programları için her iki konfigürasyonda elde edilen ortalama IPC değerlerini sunmaktadır. Üç yollu paralel yürütme yapısının, özellikle bağımsız aritmetik işlemlerin yoğun olduğu kod bloklarında tek yollu yapıya göre sağladığı hızlanma faktörü analiz edilmiştir.
