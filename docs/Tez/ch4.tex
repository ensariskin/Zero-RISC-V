...
Bu bölümde, tasarlanan süperölçekli RISC-V işlemcisine uygulanan isteğe bağlı yedeklilik tekniği detaylı olarak ele alınmaktadır. Öncelikle hata toleransının önemi ve mevcut yöntemlerin sınırlılıkları açıklanmakta, ardından tasarlanan işlemcinin bu alandaki katkıları sunulmaktadır. Bu çalışma, daha önce yayınlanan bir literatür taramasında önerilen araştırma yönünün uygulamasını oluşturmaktadır \cite{iskin_fault_tolerance_2024}.
...
Bölüm \ref{sec:fault_tolerance}'te açıklandığı gibi, geçici hatalar (SEU, MBU) ve kalıcı hatalar farklı koruma yöntemleri gerektirmektedir. Uzaysal yedeklilik (TMR, DCLS/TCLS), zamansal yedeklilik ve bilgi yedekliliği (ECC) bu amaçla geliştirilmiş temel yaklaşımlardır. Bu bölümde, mevcut yöntemlerin sınırlılıkları analiz edilmekte ve tasarlanan işlemcinin bu sınırlılıklara nasıl çözüm sunduğu açıklanmaktadır.
...
Mevcut literatürde, boruhattı seviyesinde TMR uygulayan DuckCore gibi tasarımlar \cite{li_duckcore}, çekirdek seviyesinde ODMR uygulayan Trikarenos gibi tasarımlar \cite{trikarenos_2023} ve yüksek performanslı açık kaynak RISC-V çekirdekleri olan BOOM \cite{sonicboom} ayrı ayrı bulunmaktadır. Ancak bu yaklaşımların birleştirilmesi daha önce gerçekleştirilmemiştir.
...
Tasarlanan mimari, gelecekte otomatik mod geçişini destekleyecek şekilde planlanmıştır. Bu özellik henüz gerçeklenmemiş olsa da, altyapı şu senaryoyu destekleyecek şekilde tasarlanmıştır:
...
\begin{itemize}
    ...
    \item RAT Checkpoint işaretçileri
\end{itemize}
...
ECC uygulamasının bu tez kapsamı dışında tutulması kararlaştırılmış olup, bu konu gelecek çalışmalar için önemli bir araştırma başlığı olarak önerilmektedir.

%------------------------------------------------------------------------
\section{RAT Checkpoint ile Kritik Hata Toparlanması}\label{sec:rat_checkpoint_recovery}
%------------------------------------------------------------------------

Bölüm \ref{subsec:rat_checkpoint}'te açıklanan RAT Checkpoint, yanlış dal tahmini durumunda hızlı toparlanma için tasarlanmıştır. Ancak bu yapı, güvenli modda kritik hataların toparlanması için de kullanılabilmektedir. Bu yaklaşım, mevcut donanım altyapısının hata toleransı amacıyla yeniden kullanılmasının somut bir örneğini oluşturmaktadır.

\subsection{RAT Checkpoint'in İkili Kullanımı}\label{subsec:rat_checkpoint_dual_use}

RAT Checkpoint, normal modda her dallanma komutu için checkpoint almaktadır. Ancak güvenli modda hatalar yalnızca yanlış tahmin kaynaklı değil, aynı zamanda SEU ve MBU gibi radyasyon etkilerinden de kaynaklanabilmektedir.

RAT Checkpoint, diğer hevesli yanlış tahmin toparlanma yöntemlerine göre daha fazla alan gerektirmektedir. Bununla birlikte, aynı yapının radyasyon kaynaklı hataların toparlanmasında da kullanılabilmesi, bu ek alan maliyetini haklı çıkarmaktadır. Tek bir donanım yapısı ile hem dal spekülasyonu hem de hata toleransı sağlanmaktadır.

\subsection{Güvenli Mod Checkpoint Stratejisi}\label{subsec:secure_checkpoint}

Normal modda yalnızca dallanma ve atlama komutları için anlık görüntü alınmaktadır. Güvenli modda ise önerilen yaklaşım, \textbf{her komut için anlık görüntü almaktır}. Bu strateji sayesinde:

\begin{itemize}
    \item Herhangi bir komutta kritik hata tespit edildiğinde, o komutun durumuna geri dönülebilmektedir.
    \item Toparlanma gecikmesi minimize edilmektedir çünkü en güncel anlık görüntü her zaman mevcuttur.
    \item Yanlış tahmin toparlanması ile aynı mekanizma kullanılmaktadır.
\end{itemize}

On altı girişlik RAT Checkpoint kapasitesi, bu strateji için yeterlidir. Güvenli modda IPC değeri 1 olduğundan ve komutlar sıralı olarak işlendiğinden, boruhattı derinliği kadar checkpoint yeterli olmaktadır.

\subsection{Kritik Hata Tespit Mekanizması ve Önerilen Toparlanma Akışı}\label{subsec:fatal_recovery_flow}
...
Tasarımda, boruhattının farklı aşamalarına dağılmış TMR modülleri kritik hata tespit ettiğinde bir sinyal üretmektedir. Mevcut tasarımda bu sinyal, hatanın raporlanmasını sağlamakta ve önerilen toparlanma mekanizması için bir tetikleyici görevi görmektedir. Önerilen toparlanma akışı şu şekilde tasarlanmıştır:
...
\begin{enumerate}
    ...
    \item \textbf{İlgili komutun belirlenmesi:} Hatalı komuta atanmış fiziksel yazmaç adresi, RAT Checkpoint'teki ilgili anlık görüntüye erişmek için kullanılır.
    
    \item \textbf{Anlık görüntüden geri yükleme:} RAT Checkpoint'ten yazmaç takma ad tablosu anlık görüntüsü ve program sayacı değeri alınır.
...
Bu mekanizmanın tam otonom çalışması için gereken merkezi denetleyici, bu tez kapsamında gerçeklenmemiş olup tespit ve raporlama altyapısı tamamlanmıştır. Mevcut sistemde kritik hata sinyali başarıyla üretilmekte ve simülasyon ortamına iletilmektedir. Tam otomatik toparlanma, gelecek çalışma olarak planlanmıştır.
...
\begin{itemize}
    ...
    \item \textbf{Düşük toparlanma gecikmesi:} RAT Checkpoint mekanizması sayesinde toparlanma tek saat çevriminde başlatılabilmektedir.
    
    \item \textbf{Minimal performans kaybı:} Toparlanma cezası, tipik bir yanlış tahmin cezasından daha düşüktür.
    
    \item \textbf{Mevcut altyapı kullanımı:} Ek donanım yerine, zaten mevcut olan RAT Checkpoint yapısı kullanılmaktadır.
\end{itemize}

\subsection{Fiziksel Adres Bozulması Durumu}\label{subsec:address_corruption}

Nadir durumlarda, hatalı komuta ait fiziksel yazmaç adresi de bozulabilmektedir. Bu durumda, hangi anlık görüntüye dönüleceği belirlenememektedir. Bu senaryoya karşı alternatif bir mekanizma tasarlanmıştır:

RAT Checkpoint girişleri, büyük bellek yapılarıyla birlikte ECC koruması altında olduğu varsayılmaktadır. Ayrıca, RAT Checkpoint'in baş işaretçisinde bulunan ve belirli bir zaman aşımı süresince sonuçlanmayan komutlar, potansiyel olarak bozulmuş kabul edilebilmektedir.

Bu durumda bile, RAT Checkpoint'teki en eski geçerli anlık görüntü ve doğru program sayacı değeri kullanılarak program en az kayıpla yeniden başlatılabilmektedir. Bu yaklaşım, tamamen sistem yeniden başlatmasına kıyasla çok daha hızlı ve verimli bir toparlanma sağlamaktadır.

Zaman aşımı tabanlı mekanizmanın testi henüz gerçekleştirilmemiş olmakla birlikte, bu konu gelecek çalışmalar için önerilmektedir.

\subsection{Dinamik Mod Geçişi Potansiyeli}\label{subsec:dynamic_mode_potential}

Tasarlanan mimari, henüz gerçeklenmemiş olsa da, gelecekte dışarıdan müdahale gerektirmeden otomatik mod geçişini destekleyecek şekilde planlanmıştır:
...
\begin{enumerate}
    ...
    \item \textbf{RAT Checkpoint ile kritik hata toparlanması:} Yanlış tahmin toparlanması için tasarlanan RAT Checkpoint yapısı, radyasyon kaynaklı kritik hataların toparlanmasında da kullanılmaktadır. Tek donanım yapısı ile ikili işlevsellik sağlanmaktadır.
